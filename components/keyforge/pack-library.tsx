...
const COLLECTION_LABELS: Record<SoundCollection, string> = {
  all: "All",
  "real-switches": "Real switches",
  playful: "Playful",
  "earlier-synths": "Earlier synths",
  imported: "Imported",
};
...
export function PackLibrary({ packs, pendingAction, onImport, onSelect }: PackLibraryProps) {
  ...
  const emptyMessage = collection === "all"
    ? "No instruments installed."
    : `No ${COLLECTION_LABELS[collection].toLowerCase()} instruments installed.`;
        ...
        {visiblePacks.map((pack, index) => (
          <article className="pack-row" key={pack.id}>
            ...
            <div className="pack-identity">
              <h3>{pack.name}</h3>
              <p>
                {pack.groupCounts.normal} {pack.groupCounts.normal === 1 ? "voice" : "voices"}
                {pack.bundled ? " · Included" : " · Local import"}
              </p>
            </div>
